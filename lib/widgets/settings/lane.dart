...
class LaneHeader extends StatelessWidget {
  ...
  Future<void> _delete(BuildContext context) async {
    lane != 0 ? navPageDown?.call() : (numLanes == 1 ? doNothing() : navPageUp?.call());
    await appInfo.removeLane(config, context, lane);
  }

  Future<void> _dupe() => appInfo.dupeLane(config, lane);
  ...
  Widget build(BuildContext context) => pages(config)
      ...
      : MenuAnchor(
          ...
          menuChildren: <Widget>[
            ...
            // Edit
            MenuItemButton(onPressed: () => _edit(context), child: EzIcon(config, Icons.edit)),
